...
const OurSolution = () => {
  ...
  useEffect(() => {
    const circleAnimation = () => {
      ...
      circleWrapper.style.background = `conic-gradient(#2F2F2F ${
        percentage + 15
      }%, #1A1A1A 0%)`;
    };
    ...
    <div className="bg-(--darkblack) text-(--whitelight) py-30 relative mb-[00px] ">
      <div className="container mx-auto  ">
        <div className="grid lg:grid-cols-3 grid-cols-1 gap-10 px-10 ">
          <h2 className="text-6xl font-bold lg:mb-12   py-5   lg:text-end    tracking-wider">
            Our Solution <br />
            Process
          </h2>
          ...
          <p className=" text-(--gray)  lg:flex  md:items-end font-bold">
            Digital agencies can vary in size and specialization. Some may focus
            on specific niches, such as healthcare or e-commerce, while others
            may offer a comprehensive.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1 items-start relative mt-20  ">
          ...
          >
            <div
              id="circle-wrapper"
              className="circle-wrapper h-[340px] w-[340px] rounded-[50%] border-[4px] border-[#2F2F2F] lg:flex justify-center items-center  "
            >
              <div className="circle-content h-[320px] w-[320px] bg-[#1A1A1A] rounded-[50%] lg:flex flex-col justify-center items-center">
          ...
          <div id="process-cards" className="border-l border-(--gray) relative">
            <div className="w-full max-w-[600px] pl-20 pr-20 z-50  ">
              {blurbContent.map((item, index) => (
                <div
                  key={index}
                  className="blurb-item pb-20 bg-(--darkblack) sticky top-20 pt-10"
                >
                  <img
                    src={item.image}
                    alt={item.title}
                    className="filter invert brightness-0 contrast-100 mb-8"
                  />
                  ...
                  <ul>
                    {item.additionalText.map((text, idx) => (
                      <li
                        className="lg:flex gap-2 items-center text-(--gray)"
                        key={idx}
                      >
                        <FiPlus /> {text}
                      </li>
